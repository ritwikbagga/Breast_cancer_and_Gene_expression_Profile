...
def main():
    ...
    md3 = 5
    N = 150  # Each part will be tried with 1 to 150 estimators
    print("running for model AdaBoost")
    for i in range(1, N+1):
        x_arr.append(i)
    # Train AdaBoost with max_depth = 1 recording the errors (errors will be of size 150)
        dtc = DecisionTreeClassifier(max_depth=md1)
        clf1 = AdaBoostClassifier(n_estimators=i, base_estimator=dtc, learning_rate=0.1)
        clf1.fit(train_X, train_y)
        y_pred = clf1.predict(test_X)
        accuracy = metrics.accuracy_score(test_y, y_pred)
        err_m1.append(1-accuracy)
    # Train AdaBoost with max_depth = 3 recording the errors (errors will be of size 150)
        dtc = DecisionTreeClassifier(max_depth=md2)
        clf2 = AdaBoostClassifier(n_estimators=i, base_estimator=dtc, learning_rate=0.1)
        clf2.fit(train_X, train_y)
        y_pred = clf2.predict(test_X)
        accuracy = metrics.accuracy_score(test_y, y_pred)
        err_m2.append(1 - accuracy)
    # Train AdaBoost with max_depth = 5 recording the errors (errors will be of size 150)
        dtc = DecisionTreeClassifier(max_depth=md3)
        clf3 = AdaBoostClassifier(n_estimators=i, base_estimator=dtc, learning_rate=0.1)
        clf3.fit(train_X, train_y)
        y_pred = clf3.predict(test_X)
        accuracy = metrics.accuracy_score(test_y, y_pred)
        err_m3.append(1 - accuracy)
    ...
    plt.plot(x_arr, err_m1, c='r', label=f"max_depth = {md1}")
    plt.plot(x_arr, err_m2, c='g', label=f"max_depth = {md2}")
    plt.plot(x_arr, err_m3, c='b', label=f"max_depth = {md3}")
    plt.ylabel("Test Classification Error")
    plt.xlabel("# of Trees (n_estimators)")
    plt.legend(loc=1)
    plt.ylim(0,1)
